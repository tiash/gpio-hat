...
include Expert
  ...
  let fixed_pins = 
    ...
    |`High-> Constant true
    | `Low -> Constant false in
    let t = make_pin ~name ~pin ~kind in t.pin,t)
    |> Gpio_hat.Pin.Map.of_alist_exn

     
  let create ~name ~pin ~kind value =
     let pin = make_pin ~name ~pin ~kind in
     { pins =  Map.add_exn fixed_pins ~key:pin.pin ~data:pin
     ;  value }
  ...
  let return value = 
    { pins = fixed_pins; value }

  let both a b = 
    { pins = 
      merge_pins (merge_pins a.pins b.pins) fixed_pins
; value = a.value, b.value }

  let map t ~f =
   { pins = merge_pins t.pins fixed_pins; value = f t.value }
  ...
  let rec all = function
    | [] -> return []
    | hd::tl ->
      map (both hd (all tl)) ~f:(fun (hd,tl) -> hd::tl)
end
