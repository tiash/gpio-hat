...
let model =
  Model.create "74161" ~aliases:[ "74LS161" ]
    ...
    (let%map_open.Dip16 n =
       all [ input "N_4" 6; input "N_3" 5; input "N_2" 4; input "N_1" 3 ] >>| Util.uint
     and q =
       all [ output "Q_4" 11; output "Q_3" 12; output "Q_2" 13; output "Q_1" 14 ] >>| Util.uint'
     and clear = input' "~CLR" 1 >>| (fun f v -> f (not v))
     and clock = input' "CLK" 2
     and enp = input "ENP" 7
     and ent = input "ENT" 10
     and load = input' "~LOAD" 9 >>| (fun f v -> f (not v))
     and carry = output "CARRY" 15
     in
