...
let gate n g a b y0 y1 y2 y3 =
  ...
  and n =
    all [ input (sprintf "B_%s" n) b; input (sprintf "A_%s" n) a ]
    >>| Logic.unsigned_int
  and y =
    all
      [
...
let model =
  Model.create "74139" ~aliases:[ "74LS139" ] ~summary:"Quad 2-Input NAND Gate"
    ~description:""
    (Pins.combine [ gate "1" 1 2 3 4 5 6 7; gate "2" 15 14 13 12 11 10 9 ])
